...
function VideoModal({
  ...
  // Pause and reset when modal unmounts
  useEffect(() => {
    return () => {
      if (videoRef.current) {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-[150] flex items-center justify-center bg-black/92 backdrop-blur-sm px-4"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      ...
      onClick={onClose}
    >
      {/* Portrait container — 9:16, max 380px wide so it fits any screen */}
      <motion.div
        className="relative rounded-2xl overflow-hidden shadow-2xl bg-black w-full"
        style={{
          maxWidth: "min(380px, calc(100vh * 9 / 16 - 32px))",
          aspectRatio: "9/16",
        }}
      ...
      >
        {/* Video mounts only when modal is open */}
        <video
          ref={videoRef}
...
export default function Showcase() {
  ...
  return (
    <>
      ...
      {/* Modal only mounts when a video is active — lazy DOM */}
      <AnimatePresence>
        {activeIndex !== null && (
          <VideoModal
